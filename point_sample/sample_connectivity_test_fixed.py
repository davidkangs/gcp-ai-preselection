...
# -*- coding: utf-8 -*-
"""
샘플 점간 연결성 분석 테스트 (유클리드 거리 버전)
목적: 도로 스켈레톤으로 연결성 판단 + 유클리드 직선 거리 계산
"""
...
def visualize_results(points_gdf, road_gdf, skeleton_lines, projected_points, connections, direct_connections):
    ...
    plt.tight_layout()
    plt.savefig('sample_connectivity_euclidean.png', dpi=300, bbox_inches='tight')
    plt.show()
    
    # 결과 요약 출력
    print("\n" + "="*60)
    print("📊 분석 결과 요약 (유클리드 거리)")
    print("="*60)
    ...
    if blocked_connections:
        ...
        for conn in blocked_connections:
            print(f"   P{conn['point1_id']} - P{conn['point2_id']}: {conn['euclidean_distance']:.1f}m (중간점 존재)")
...
def save_results(connections, direct_connections, projected_points):
    ...
    with open('sample_connectivity_euclidean_results.json', 'w', encoding='utf-8') as f:
        json.dump(results, f, ensure_ascii=False, indent=2)
    
    print(f"💾 결과가 'sample_connectivity_euclidean_results.json'에 저장되었습니다")

def main():
    """메인 실행 함수"""
    print("🚀 점간 연결성 분석 시작 (유클리드 거리 버전)")
    print("="*60)
