"""프로세스 4: AI 예측 + 보존 지적기준점 통합 (지구계 지원) """

# 🔒 독립실행형 환경 설정 (MKL 문제 해결)
import os
import sys

# MKL 문제 완전 해결을 위한 환경 변수 설정
...
# NumPy가 OpenBLAS를 사용하도록 강제
import numpy as np

# 독립실행형에서는 설정 출력 생략 (성능상 이유)
# try:
#     np.show_config()  # 설정 확인용
# except:
#     pass
...
try:
    ...
    logger.info("✅ 런타임 패치 적용 완료")
except Exception as patch_e:
    logger.warning(f"⚠️ 런타임 패치 적용 실패: {patch_e}")
...
class InferenceTool(QMainWindow):
    ...
    def __init__(self):
        ...
        # 지구계 관련 추가
        self.file_mode = 'district'  # 'district' or 'road' - 지구계를 기본으로 설정
        ...
        self.user_deleted_points = []
        self.overlap_analysis_result = None
        
        # AI 초기 검출 개수 저장
        self.initial_ai_count = 0
        self.ai_points_backup = None
        ...
        # 지구계 모드 기본 설정 (UI 일관성 보장)
        self.set_file_mode('district')
        
        # 자동으로 수동 편집 모드 활성화
        self.enable_manual_edit_mode()
        
        # 🚀 투명 캐싱 시스템 초기화 (백그라운드에서 자동 준비)
        try:
            ...
            logger.info(f"💾 캐시 시스템 초기화 중... 경로: {cache_dir}")
            
            # 캐시 디렉토리 생성 확인
            if cache_dir.exists():
                logger.info(f"✅ 캐시 디렉토리 존재: {cache_dir}")
                
                # 기존 캐시 파일 개수 확인
                skeleton_cache_count = len(list((cache_dir / "skeleton").glob("*.pkl")))
                ai_cache_count = len(list((cache_dir / "ai_prediction").glob("*.pkl")))
                clipping_cache_count = len(list((cache_dir / "clipping").glob("*.pkl")))
                
                logger.info(f"📂 기존 캐시: 스켈레톤 {skeleton_cache_count}개, AI예측 {ai_cache_count}개, 클리핑 {clipping_cache_count}개")
            else:
                logger.info(f"🆕 새 캐시 디렉토리 생성: {cache_dir}")
            
            # 30일 지난 캐시 자동 정리
            cache.clear_old_cache(max_age_days=30)
            logger.info("🧹 오래된 캐시 정리 완료")
            
        except Exception as e:
            logger.warning(f"⚠️ 캐시 시스템 초기화 실패 (기능은 정상 작동): {e}")

    def init_ui(self):
        self.setWindowTitle("도로망 AI 예측 및 수정 - 프로세스 4 (보존 지적기준점 통합)")
        self.setGeometry(100, 50, 1200, 750)  # 노트북 환경에 맞게 크기 조정
        ...
        # 보존 기준점 표시 관련 초기화
        if not hasattr(self.canvas_widget.canvas, 'reference_points_display'):
            self.canvas_widget.canvas.reference_points_display = []
        if not hasattr(self.canvas_widget.canvas, 'show_reference_points'):
            self.canvas_widget.canvas.show_reference_points = True
        
        # Canvas 신호 연결 (점 변경 시 자동 업데이트)
        self.canvas_widget.canvas.point_added.connect(lambda cat, x, y: self.update_point_count_table())
        self.canvas_widget.canvas.point_removed.connect(lambda cat, x, y: self.update_point_count_table())
        
        splitter.addWidget(self.canvas_widget)
        
        splitter.setSizes([250, 950])  # 노트북 환경에 맞게 비율 조정
    ...
    def create_left_panel(self):
        ...
        # ===== 1. 파일 모드 & 좌표계 선택 (나란히 배치) =====
        mode_crs_layout = QHBoxLayout()
        ...
        self.district_radio = QRadioButton("지구계 파일")
        self.district_radio.setChecked(True)  # 지구계 파일을 기본 선택으로 변경
        ...
        self.road_radio = QRadioButton("도로망 파일")
        self.road_radio.setChecked(False)  # 기본 선택 해제
        self.mode_button_group.addButton(self.road_radio)
        mode_layout.addWidget(self.road_radio)
        
        mode_group.setLayout(mode_layout)
        mode_crs_layout.addWidget(mode_group)
        
        # 좌표계 선택 (지구계 모드에서만 표시)
        self.crs_group = QGroupBox("좌표계")
        crs_layout = QVBoxLayout()
        
        self.crs_button_group = QButtonGroup()  # 버튼 그룹 추가
        
        self.crs_5185_radio = QRadioButton("서부원점 (EPSG:5185)")
        self.crs_5185_radio.setChecked(False)
        self.crs_button_group.addButton(self.crs_5185_radio)
        self.crs_5185_radio.toggled.connect(self.on_crs_changed)
        crs_layout.addWidget(self.crs_5185_radio)
        
        self.crs_5186_radio = QRadioButton("중부원점 (EPSG:5186)")
        self.crs_5186_radio.setChecked(True)
        self.crs_button_group.addButton(self.crs_5186_radio)
        self.crs_5186_radio.toggled.connect(self.on_crs_changed)
        crs_layout.addWidget(self.crs_5186_radio)
        
        self.crs_5187_radio = QRadioButton("동부원점 (EPSG:5187)")
        self.crs_5187_radio.setChecked(False)
        self.crs_button_group.addButton(self.crs_5187_radio)
        self.crs_5187_radio.toggled.connect(self.on_crs_changed)
        crs_layout.addWidget(self.crs_5187_radio)
        
        self.crs_5188_radio = QRadioButton("동해원점 (EPSG:5188)")
        self.crs_5188_radio.setChecked(False)
        self.crs_button_group.addButton(self.crs_5188_radio)
        self.crs_5188_radio.toggled.connect(self.on_crs_changed)
        crs_layout.addWidget(self.crs_5188_radio)
        
        self.crs_group.setLayout(crs_layout)
        self.crs_group.setVisible(True)  # 기본으로 표시 (지구계 모드가 기본이므로)
        mode_crs_layout.addWidget(self.crs_group)
        
        layout.addLayout(mode_crs_layout)
        ...
        # 상단 최종 개수 표시
        self.final_count_label = QLabel("🎯 최종 선점 기준점: 0개")
        self.final_count_label.setStyleSheet("""
            QLabel {
        ...
        """)
        result_layout.addWidget(self.final_count_label)
        
        # 신뢰도는 고정값 0.7 사용 (슬라이더 제거됨)
        
        self.result_label = QLabel("분석 전...")
        self.result_label.setStyleSheet("QLabel {padding: 5px; background-color: #f0f0f0; border-radius: 3px; font-size: 12px;}")
        self.result_label.setMaximumHeight(35)  # 높이 제한
        ...
        # 보존 지적기준점 업로드 버튼 추가
        excel_btn = QPushButton("📊 보존 지적기준점 Excel/SHP 업로드")
        excel_btn.clicked.connect(self.upload_reference_points)
        ...
        self.point_count_table.setRowCount(6)
        self.point_count_table.setItem(0, 0, QTableWidgetItem("AI 기준점"))
        self.point_count_table.setItem(1, 0, QTableWidgetItem("보존 지적기준점"))
        self.point_count_table.setItem(2, 0, QTableWidgetItem("선점 기준점"))
        self.point_count_table.setItem(3, 0, QTableWidgetItem("- 참조"))
        self.point_count_table.setItem(4, 0, QTableWidgetItem("- 삭제"))
        self.point_count_table.setItem(5, 0, QTableWidgetItem("- 추가"))
        ...
        self.point_count_table.setItem(3, 1, QTableWidgetItem("0점"))
        self.point_count_table.setItem(4, 1, QTableWidgetItem("0점"))
        self.point_count_table.setItem(5, 1, QTableWidgetItem("0점"))
        self.point_count_table.setMinimumHeight(200)  # 6행에 맞게 높이 증가
        self.point_count_table.setMaximumHeight(220)  # 최대 높이도 증가
        ...
        # 실제기준점표시와 거리표시를 나란히 배치
        excel_distance_layout = QHBoxLayout()
        
        self.show_excel_checkbox = QCheckBox("보존기준점표시")
        self.show_excel_checkbox.setChecked(True)
        self.show_excel_checkbox.toggled.connect(self.toggle_reference_points)
        excel_distance_layout.addWidget(self.show_excel_checkbox)
        
        self.show_distance_checkbox = QCheckBox("거리표시")
        self.show_distance_checkbox.setChecked(True)
        self.show_distance_checkbox.toggled.connect(self.toggle_distance_display)
        excel_distance_layout.addWidget(self.show_distance_checkbox)
        
        checkbox_layout.addLayout(excel_distance_layout)
        
        result_layout.addLayout(checkbox_layout)
        
        # 3개 버튼 완전 제거됨 (모든 AI 예측 수락, 거리 계산 실행, 사용자 수정 초기화)
        ...
        # SHP 내보내기 버튼
        export_shp_btn = QPushButton("💾 결과 SHP 내보내기")
        ...
        batch_btn.setVisible(False)  # 비가시화
    ...
    def get_target_crs(self):
        """선택된 좌표계 반환"""
        if self.crs_5185_radio.isChecked():
            return 'EPSG:5185'
        elif self.crs_5187_radio.isChecked():
            return 'EPSG:5187'
        elif self.crs_5188_radio.isChecked():
            return 'EPSG:5188'
        return 'EPSG:5186'  # 기본값: 중부원점
    
    def on_crs_changed(self, checked):
        """좌표계 변경 이벤트 처리"""
        if not checked:  # toggled는 체크/언체크 모두 호출되므로 체크된 경우만 처리
            return
        
        # 현재 선택된 좌표계 업데이트
        self.selected_crs = self.get_target_crs()
        logger.info(f"좌표계 변경: {self.selected_crs}")
        
        # 캔버스에 좌표계 설정
        self.canvas_widget.canvas.crs = self.selected_crs
        
        # 이미 로드된 폴리곤 데이터가 있으면 좌표계 변환해서 재표시
        # (전체 분석을 다시 실행하지 않음!)
        if self.current_polygon_data:
            self.reload_with_new_crs()
    
    def reload_with_new_crs(self):
        """좌표계 변경 시 데이터 재표시"""
        try:
            # 안전장치: 필수 데이터 존재 확인
    ...
    def select_file(self):
        ...
        if file_path:
            ...
            # 캔버스에 현재 좌표계 설정
            self.canvas_widget.canvas.crs = self.get_target_crs()
    ...
    def process_file(self):
        ...
        if self.file_mode == 'district':
            # 현재 선택된 좌표계로 처리
            self.process_district_file(self.current_file, force_crs=self.get_target_crs())
    ...
    def process_road_file(self, file_path):
        ...
        try:
            ...
            if result['success']:
                # ✅ 캐싱 활성화 - 필터링 완료된 상태 우선 로드
                cached_edit_state = self.try_load_cached_edit_state()
                if cached_edit_state:
                    logger.info("✅ 도로망 캐시 복원 - 필터링 건너뛰기")
                    self.apply_cached_edit_state(cached_edit_state)
                    # 🛡️ 거리 계산은 항상 새로 실행
                ...
                # 🆕 신규 도로망 파일 → 필터링 적용!
                progress.setLabelText("🔧 점 최적화 중...")
                progress.setValue(80)
                
                try:
                    total_removed, final_points = self.run_multi_stage_optimization()
                    logger.info(f"🎯 도로망 필터링 완료: {total_removed}개 제거, {final_points}개 점 유지")
                except Exception as e:
                    logger.error(f"필터링 오류 (계속 진행): {e}")
                
                # 🛡️ 거리 계산은 항상 새로 실행 (캐시되지 않음)
                self.calculate_and_display_distances()
                
                # ✅ 필터링 완료된 최종 상태 캐시 저장
                self.save_current_edit_state()
                
                # 결과 표시 (필터링 후 최종 개수)
                final_total_points = sum(len(self.canvas_widget.canvas.points.get(cat, [])) for cat in ['intersection', 'curve', 'endpoint'])
                
                # AI 초기 개수 저장 (필터링 후 최종 개수)
                if self.initial_ai_count == 0:
                    self.initial_ai_count = final_total_points
                    logger.info(f"AI 초기 개수 저장 (필터링 후): {self.initial_ai_count}개")
                
                self.statusBar().showMessage(f"도로망 처리 완료 - {final_total_points}개 점 (필터링 적용)")
                self.result_label.setText(f"처리 완료: {final_total_points}개 점")
            else:
    ...
    def process_district_file(self, district_file, force_crs=None):
        """지구계 파일 처리 (리팩토링된 버전)
        
        Args:
            district_file: 지구계 파일 경로
            force_crs: 강제로 사용할 좌표계 (None이면 현재 선택된 좌표계 사용)
        """
        try:
            ...
            # 사용할 좌표계 결정 (force_crs가 있으면 우선 사용)
            target_crs = force_crs if force_crs else self.get_target_crs()
            logger.info(f"지구계 처리 - 사용 좌표계: {target_crs}")
            ...
            # 첫 번째 폴리곤 로드
            # polygon_results가 있으면 우선 사용
            if 'polygon_results' in results and results['polygon_results']:
                # 새로운 파이프라인 매니저 구조
                self.load_polygon_result(results)
            elif 'polygons' in results and results['polygons']:
                # 기존 구조
                self.load_polygon_result(results)
            else:
                logger.warning("폴리곤 데이터가 없습니다")
    ...
    def process_with_manual_road(self, polygons, road_folder):
        ...
        try:
            ...
            poly_data = first_polygon.get('geometry_gdf', first_polygon['geometry'])
    ...
    def load_polygon_result(self, result):
        ...
        logger.info(f"🔍 load_polygon_result 호출됨 - result keys: {list(result.keys())}")
        
        if not result['success']:
            logger.info("❌ result['success'] = False, 종료")
            return
        
        # ✅ 캐싱 활성화 - 필터링 완료된 상태 우선 로드
        cached_edit_state = self.try_load_cached_edit_state()
        if cached_edit_state:
            logger.info("✅ 캐시된 사용자 편집 상태 복원됨 - 필터링 건너뛰기")
            # 캐시된 상태 적용
            self.apply_cached_edit_state(cached_edit_state)
            
            # AI 초기 개수 설정 (캐시 복원 후)
            if self.initial_ai_count == 0:
                self.initial_ai_count = sum(len(self.canvas_widget.canvas.points.get(cat, [])) for cat in ['intersection', 'curve', 'endpoint'])
                logger.info(f"AI 초기 개수 저장 (캐시 복원): {self.initial_ai_count}개")
            
            # 지구계 정보는 캐시와 별개로 항상 재설정
            if 'polygons' in result:
                current_polygon = result['polygons'][self.current_polygon_index]
                geom_data = None
                
                # geometry_gdf가 있으면 우선 사용
                if 'geometry_gdf' in current_polygon and current_polygon['geometry_gdf'] is not None:
                    poly_gdf = current_polygon['geometry_gdf']
                    if not poly_gdf.empty:
                        geom_data = poly_gdf.geometry.iloc[0]
                elif 'geometry' in current_polygon:
                    geom_data = current_polygon['geometry']
                
                if geom_data:
                    self.canvas_widget.set_background_data(geom_data)
            elif 'polygon_results' in result and result['polygon_results']:
                # 새로운 파이프라인 결과에서 지구계 정보 가져오기
                polygon_result = result['polygon_results'][self.current_polygon_index] if self.current_polygon_index < len(result['polygon_results']) else result['polygon_results'][0]
                if 'polygon_info' in polygon_result and 'geometry' in polygon_result['polygon_info']:
                    self.canvas_widget.set_background_data(polygon_result['polygon_info']['geometry'])
            
            # 🛡️ 거리 계산은 항상 새로 실행 (실시간 정보 유지)
            self.calculate_and_display_distances()
            return
            
        # 새로운 파이프라인 매니저 결과 구조 처리
        if 'polygon_results' in result:
            logger.info("🔍 파이프라인 매니저 결과 구조 처리 중...")
            # 파이프라인 매니저에서 온 결과 - 이미 처리된 데이터
            if not result['polygon_results']:
                logger.info("❌ polygon_results 비어있음")
                return
            
            # 현재 폴리곤 인덱스의 결과 사용 (안전성 체크 포함)
            if self.current_polygon_index < len(result['polygon_results']):
                polygon_result = result['polygon_results'][self.current_polygon_index]
            else:
                logger.warning(f"폴리곤 인덱스 범위 초과: {self.current_polygon_index} >= {len(result['polygon_results'])}")
                polygon_result = result['polygon_results'][0]  # 폴백
            ...
            if 'skeleton' in polygon_result:
                ...
                # AI 예측 결과도 저장 (있는 경우)
                if 'ai_points' in polygon_result:
                    self.canvas_widget.canvas.ai_points = polygon_result['ai_points']
                
                # 화면 업데이트
                self.canvas_widget.update_display()
                self.update_point_count_table()
                
                # 🆕 처음 여는 파일 → 필터링 적용 (캐시 복원 시에는 건너뛰어짐)
                try:
                    total_removed, final_points = self.run_multi_stage_optimization()
                    logger.info(f"신규 파일 필터링 완료: {total_removed}개 제거, {final_points}개 점 유지")
                except Exception as e:
                    logger.error(f"필터링 오류 (계속 진행): {e}")
                    # 필터링 실패해도 계속 진행
                ...
                self.result_label.setText(f"처리 완료: {final_total_points}개 점")
                logger.info(f"신규 파일 처리 완료: AI 분석 + 필터링 → {final_total_points}개 점")
                return
        
        # 기존 구조 처리 (polygons 키 사용)
        if not result.get('polygons'):
            logger.info("❌ polygons 키 없음")
            return
        
        logger.info("🔍 기존 구조 (polygons) 처리 중...")
        ...
        if 'clipped_road' in current_polygon and current_polygon['clipped_road'] is not None:
            ...
            try:
                ...
                with tempfile.TemporaryDirectory() as temp_dir:
                    ...
                    temp_path = os.path.join(temp_dir, "temp_road.gpkg")
                    ...
                    # 🚀 투명 캐싱: 스켈레톤 추출 결과 캐싱
                    from src.utils.cache_manager import cache_skeleton_extraction
                    import hashlib
                    
                    def skeleton_extraction_logic():
                        # 기존 스켈레톤 추출 로직 (변경 없음)
                        skeleton_extractor = SkeletonExtractor()
                        return skeleton_extractor.process_shapefile(temp_path)
                    
                    # 도로 데이터 해시를 포함한 캐시 키 생성
                    road_hash = hashlib.md5(str(road_gdf.bounds.values.tolist()).encode()).hexdigest()[:16]
                    cache_params = {"road_hash": road_hash}
                    
                    # 캐싱된 스켈레톤 추출 (district 파일 기준)
                    if self.current_file:
                        skeleton, intersections = cache_skeleton_extraction(
                            self.current_file, skeleton_extraction_logic, **cache_params
                        )
                    else:
                        skeleton, intersections = skeleton_extraction_logic()
                ...
                # 2-2단계: AI 기반 끝점 검출
                    ...
                    # 2-3단계: 🚀 v1 최적화 - 간단한 휴리스틱 커브 검출 우선 사용
                    logger.info("🔄 간단한 휴리스틱 커브 검출 사용 (성능 최적화)")
                    
                    # 스켈레톤 포인트 수에 따라 처리 방식 결정
                    if len(skeleton) > 2000:
                        # 큰 데이터셋: 간단한 휴리스틱만 사용
                        logger.info(f"⚡ 큰 데이터셋({len(skeleton)}점) - 간단한 처리 모드")
                        curves = self.detect_heuristic_curves(skeleton, curvature_threshold=0.4)
                    else:
                        # 작은 데이터셋: 경계선 기반 검출 사용
                        logger.info(f"🔄 일반 데이터셋({len(skeleton)}점) - 경계선 기반 검출")
                        boundary_curves = self.detect_boundary_based_curves(
                            skeleton, 
                            sample_distance=20.0,      # 샘플링 거리 증가 (성능 향상)
                            curvature_threshold=0.25,  # 임계값 증가 (노이즈 감소)
                            road_buffer=3.0,           # 도로 버퍼
                            cluster_radius=25.0        # 군집 반경 증가 (처리량 감소)
                        )
                    ...
                    self.result_label.setText(f"최종 {final_points}개 점")
                    msg = f"지구계 다단계 최적화 완료 - 총 {final_points}개 점 유지, {total_removed}개 제거"
                    self.statusBar().showMessage(msg)
                    
                    # 자동 거리 계산 제거 - 버튼으로만 수동 실행
            except Exception as e:
                if 'progress' in locals():
                    progress.close()
                ...
                total_points = len(intersections)  # 기본 모드에서는 교차점만
                self.result_label.setText(f"최종 {total_points}개 점")
    ...
    def update_polygon_navigation(self):
        ...
        # 디버깅: 데이터 구조 확인
        logger.info(f"🔍 폴리곤 데이터 구조 확인: {list(self.current_polygon_data.keys())}")
        
        # 안전하게 total_polygons 확인
        total = self.current_polygon_data.get('total_polygons', 0)
        
        # 새로운 구조와 기존 구조 모두 지원
        polygons = self.current_polygon_data.get('polygons', [])  # 기존 구조
        polygon_results = self.current_polygon_data.get('polygon_results', [])  # 새 구조
        processed_polygons = self.current_polygon_data.get('processed_polygons', 0)  # 처리된 폴리곤 수
        
        # 실제 폴리곤 수 계산
        if polygon_results:
            actual_total = len(polygon_results)
        elif polygons:
            actual_total = len(polygons)
        elif processed_polygons > 0:
            actual_total = processed_polygons
        else:
            actual_total = 0
        
        # 디버깅 정보 출력
        logger.info(f"🔍 total_polygons: {total}, polygons: {len(polygons)}, polygon_results: {len(polygon_results)}, processed: {processed_polygons}, actual_total: {actual_total}")
        
        # 실제 폴리곤 수와 비교하여 더 정확한 값 사용
        total = max(total, actual_total)
        current = self.current_polygon_index + 1
        
        logger.info(f"🔍 최종 total: {total}, current: {current}")
        
        # 폴리곤 정보가 있는 경우에만 네비게이션 표시
        if total > 1:
            ...
            # 폴리곤 네비게이션 위젯 표시
            if hasattr(self, 'polygon_nav_widget'):
                self.polygon_nav_widget.setVisible(True)
        else:
            # 단일 폴리곤인 경우 네비게이션 숨김
            if hasattr(self, 'polygon_nav_widget'):
    ...
    def next_polygon(self):
        ...
        if self.current_polygon_index < total - 1:
            old_index = self.current_polygon_index
            self.save_current_polygon_session()
            self.current_polygon_index += 1
            logger.info(f"🔄 폴리곤 전환: {old_index + 1} → {self.current_polygon_index + 1}")
            self.load_polygon_at_index(self.current_polygon_index)
            self.update_polygon_navigation()
        else:
            logger.info(f"⚠️ 마지막 폴리곤입니다. current: {self.current_polygon_index + 1}, total: {total}")

    def load_polygon_at_index(self, index):
        """특정 인덱스의 폴리곤 로드"""
        logger.info(f"📍 폴리곤 {index + 1} 로드 시작")
        ...
        logger.info(f"📊 데이터 구조: polygons={len(polygons)}, polygon_results={len(polygon_results)}")
        
        # 사용할 폴리곤 데이터 결정
        if polygon_results:
            available_polygons = polygon_results
            data_type = "polygon_results"
        elif polygons:
            available_polygons = polygons
            data_type = "polygons"
        else:
            logger.warning("❌ 사용 가능한 폴리곤 데이터가 없습니다")
            return
        
        if not available_polygons or index >= len(available_polygons):
            logger.warning(f"❌ 인덱스 범위 초과: {index} >= {len(available_polygons)}")
            return
        
        logger.info(f"✅ {data_type} 구조에서 폴리곤 {index + 1} 로드 중...")
    ...
    def run_multi_stage_optimization(self):
        """🎯 스마트 최적화: 데이터 크기에 따라 자동 선택 (정확도 vs 성능)"""
        try:
            # 🔍 스켈레톤 크기 확인
            skeleton = self.canvas_widget.skeleton
            skeleton_size = len(skeleton) if skeleton else 0
            
            logger.info(f"🔧 AI 기반 필터링 적용 4단계 - 스켈레톤: {skeleton_size}개 점")
            return self._run_precise_optimization_without_endpoints()
                
        except Exception as e:
            logger.error(f"스마트 최적화 실패: {e}")
            # 오류 발생 시 안전한 빠른 모드로 폴백
            logger.info("🔄 안전 모드로 폴백 - 빠른 필터링 실행")
            return self._run_fast_optimization()
    
    def _run_precise_optimization_without_endpoints(self):
        """💎 정교한 최적화 (4단계 제외) - AI 기반 필터링"""
        try:
            logger.info("💎 AI 기반 필터링 4단계 시작")
            
            # 1단계: 지능형 클러스터링
            clustering_result = self.intelligent_clustering_optimization()
            stage1_removed = clustering_result[0] if clustering_result else 0
            
            self.canvas_widget.canvas.draw_points_layer()
            self.canvas_widget.canvas.scene().update()
            self.update_point_count_table()
            logger.info(f"✅ 1단계 완료: {stage1_removed}개 제거")
            
            # 2단계: 클러스터링 기반 삭제
            stage2_removed = self.remove_clustered_points(12.0)  # 더 촘촘한 임계값
            
            self.canvas_widget.canvas.draw_points_layer()
            self.canvas_widget.canvas.scene().update()
            self.update_point_count_table()
            logger.info(f"✅ 2단계 완료: {stage2_removed}개 제거")
            ...
            self.canvas_widget.canvas.scene().update()
            self.update_point_count_table()
            logger.info(f"✅ 3단계 완료: {stage3_removed}개 제거 (연결성 검사)")
            
            # 4단계: 하이브리드 필터링 (자동 끝점 정리 제외)
            self.filter_overlapping_points()
            
            self.canvas_widget.canvas.draw_points_layer()
            self.canvas_widget.canvas.scene().update()
            self.update_point_count_table()
            logger.info("✅ 4단계 완료: 하이브리드 필터링")
            ...
            logger.info(f"🎉 AI 기반 필터링 완료: 총 {total_removed}개 제거, 최종 {final_points}개 점")
            return total_removed, final_points
            
        except Exception as e:
            logger.error(f"AI 기반 필터링 실패: {e}")
            self.canvas_widget.update_display()
            return 0, sum(len(self.canvas_widget.canvas.points.get(cat, [])) for cat in ['intersection', 'curve', 'endpoint'])

    def _run_precise_optimization(self):
        """💎 정교한 최적화 (작은 데이터용) - 높은 정확도"""
        try:
            logger.info("💎 정교한 4단계 최적화 시작")
            ...
            # 4단계: 자동 끝점 정리 (정교한 모드에서만)
            stage4_removed = self.auto_remove_road_endpoints()
            
            self.canvas_widget.canvas.draw_points_layer()
            self.canvas_widget.canvas.scene().update()
            self.update_point_count_table()
            logger.info(f"✅ 4단계 완료: {stage4_removed}개 제거 (끝점 정리)")
            ...
            logger.info(f"🎉 정교한 최적화 완료: 총 {total_removed}개 제거, 최종 {final_points}개 점")
            return total_removed, final_points
            
        except Exception as e:
            logger.error(f"정교한 최적화 실패: {e}")
            self.canvas_widget.update_display()
            return 0, sum(len(self.canvas_widget.canvas.points.get(cat, [])) for cat in ['intersection', 'curve', 'endpoint'])
    
    def _run_hybrid_optimization(self):
        """⚖️ 하이브리드 최적화 (중간 데이터용) - 균형"""
        try:
            logger.info("⚖️ 하이브리드 3단계 최적화 시작")
            ...
            # 2단계: 클러스터링 기반 삭제
            stage2_removed = self.remove_clustered_points(15.0)  # 표준 임계값
            ...
            # 3단계: 하이브리드 필터링만 (연결성 검사 제외)
            self.filter_overlapping_points()
            
            self.canvas_widget.canvas.draw_points_layer()
            self.canvas_widget.canvas.scene().update()
            self.update_point_count_table()
            logger.info("✅ 3단계 완료: 하이브리드 필터링")
            ...
            logger.info(f"🎉 하이브리드 최적화 완료: 총 {total_removed}개 제거, 최종 {final_points}개 점")
            return total_removed, final_points
            
        except Exception as e:
            logger.error(f"하이브리드 최적화 실패: {e}")
            self.canvas_widget.update_display()
            return 0, sum(len(self.canvas_widget.canvas.points.get(cat, [])) for cat in ['intersection', 'curve', 'endpoint'])
    
    def _run_fast_optimization(self):
        """⚡ 빠른 최적화 (큰 데이터용) - 고성능 (기존 v1 방식)"""
        try:
            logger.info("⚡ 빠른 2단계 최적화 시작")
            
            # 1단계: 지능형 클러스터링 (가벼운 중복점 제거)
            clustering_result = self.intelligent_clustering_optimization()
            stage1_removed = clustering_result[0] if clustering_result else 0
            ...
            # 2단계: 가까운 점 클러스터링 삭제 (15m 반경)
            stage2_removed = self.remove_clustered_points(18.0)  # 더 넓은 임계값 (빠른 처리)
            ...
            # ❌ 무거운 연산들 제외:
            # - remove_one_curve_point_by_connectivity() (O(n³) 복잡도)
            # - auto_remove_road_endpoints() (복잡한 경계 계산)
            ...
            logger.info(f"🎉 빠른 최적화 완료: 총 {total_removed}개 제거, 최종 {final_points}개 점")
            return total_removed, final_points
            
        except Exception as e:
            logger.error(f"빠른 최적화 실패: {e}")
            # 오류 발생 시에도 UI 업데이트
            self.canvas_widget.update_display()
            return 0, sum(len(self.canvas_widget.canvas.points.get(cat, [])) for cat in ['intersection', 'curve', 'endpoint'])
    ...
    def remove_clustered_points(self, distance_threshold=15.0):
        ...
        # 🚀 즉시 UI 업데이트 (클러스터링 삭제 결과 바로 반영)
        if deleted_count > 0:
            self.canvas_widget.canvas.draw_points_layer()
            self.canvas_widget.canvas.scene().update()
            logger.info(f"클러스터링 삭제: {deleted_count}개 점 제거 및 UI 업데이트 완료")
    ...
    def auto_remove_road_endpoints(self):
        ...
        # 타입과 내용 검증
        if not isinstance(skeleton, (list, tuple)) or len(skeleton) == 0:
            return 0
        ...
        try:
            x_coords = []
            y_coords = []
            skeleton_list = list(skeleton)  # 명시적 리스트 변환
            for pt in skeleton_list:
    ...
    def filter_overlapping_points(self):
        ...
        try:
            ...
            # 🚀 즉시 UI 업데이트 (하이브리드 필터링 결과 바로 반영)
            if removed_count > 0:
                self.canvas_widget.canvas.draw_points_layer()
                self.canvas_widget.canvas.scene().update()
                logger.info(f"스켈레톤 기반 하이브리드 중복 점 필터링 완료: {removed_count}개 점 제거 및 UI 업데이트 완료")
            else:
                logger.info("스켈레톤 기반 하이브리드 중복 점 필터링 완료: 제거된 점 없음")
                
        except Exception as e:
            logger.error(f"스켈레톤 기반 하이브리드 중복 점 필터링 오류: {e}")

    def run_ai_prediction_auto(self, skeleton, file_path):
        """자동화된 AI 예측 실행 (투명 캐싱 적용)"""
        try:
            if not self.model_path:
                return None
            
            # 🚀 투명 캐싱: 스켈레톤과 모델 기반으로 캐시 확인
            from src.utils.cache_manager import cache_ai_prediction
            
            def ai_prediction_logic(skeleton):
                # 기존 AI 예측 로직 (변경 없음)
                agent = create_agent()
            ...
            # 캐싱된 AI 예측 실행 (기존 로직과 동일한 결과)
            result = cache_ai_prediction(
                file_path, self.model_path, ai_prediction_logic, skeleton
            )
    ...
    def run_prediction(self):
        ...
        if self.file_mode == 'district' and self.current_polygon_data:
            ...
            if 'clipped_road' in current_polygon and current_polygon['clipped_road'] is not None:
                ...
                # 2) GeometryCollection 처리 후 저장
                from shapely.geometry import GeometryCollection
                clipped_gdf = current_polygon['clipped_road'].copy()
                
                # GeometryCollection을 개별 geometry로 분해
                if any(clipped_gdf.geometry.geom_type == 'GeometryCollection'):
                    new_rows = []
                    for idx, row in clipped_gdf.iterrows():
                        if isinstance(row.geometry, GeometryCollection):
                            ...
                            new_rows.append(row)
                    clipped_gdf = gpd.GeoDataFrame(new_rows, crs=current_polygon['clipped_road'].crs)
                
                clipped_gdf.to_file(temp_path, driver='GPKG')
    ...
    def on_prediction_completed(self, result):
        ...
        if result['success']:
            ...
            # 지구계 30m 반경 외부 점들 자동 필터링
            self.filter_points_by_district_boundary()
            
            self.statusBar().showMessage(f"AI 예측 + 다단계 최적화 + 지구계 필터링 완료 (총 {total_removed}개 제거, 최종 {final_points}개 유지)", 5000)
    ...
    def on_prediction_error(self, error_msg):
        ...
        if hasattr(self.prediction_worker, 'temp_path') and self.prediction_worker.temp_path:
            try:
                Path(self.prediction_worker.temp_path).unlink(missing_ok=True)
    ...
    def toggle_reference_points(self, checked):
        """보존 지적기준점 표시/숨김 토글"""
        self.canvas_widget.canvas.show_reference_points = checked
        self.canvas_widget.canvas.update_display()
        logger.info(f"보존 지적기준점 표시: {'ON' if checked else 'OFF'}")

    def toggle_distance_display(self, checked):
        """거리 표시 토글"""
        self.canvas_widget.canvas.toggle_distance_layer(checked)
        logger.info(f"거리 표시: {'ON' if checked else 'OFF'}")

    def accept_all_predictions(self):
        """AI 예측 수락 + 연결성 검사 기반 점 삭제 기능 (일괄 처리)"""
        # 1단계: AI 기반 삭제 준비
        ai_delete_count = 0
        ai_delete_attempts = len(self.canvas_widget.canvas.ai_points.get('delete', []))
        
        logger.info(f"🗑️ AI 기반 일괄 삭제 시작: {ai_delete_attempts}개 점")
        
        # 🚀 일괄 삭제: 각 삭제마다 UI 업데이트하지 않음
        for i, delete_point in enumerate(self.canvas_widget.canvas.ai_points.get('delete', [])):
            ...
            # 🚀 UI 업데이트 없이 점만 삭제 (내부 로직만 실행)
            min_dist = float('inf')
            remove_category = None
            remove_idx = None
            
            # 가장 가까운 점 찾기
            for category, points in self.canvas_widget.canvas.points.items():
                for j, (px, py) in enumerate(points):
                    dist = np.sqrt((delete_point[0] - px)**2 + (delete_point[1] - py)**2)
                    if dist < min_dist:
                        min_dist = dist
            ...
            if remove_category and remove_idx is not None and min_dist < 30:
                removed = self.canvas_widget.canvas.points[remove_category].pop(remove_idx)
                self.canvas_widget.canvas.deleted_points[remove_category].append(removed)
                ai_delete_count += 1
                logger.info(f"✅ AI 삭제 성공: {remove_category} 카테고리에서 1개 점 제거")
            else:
        ...
        # 🚀 모든 삭제 완료 후 한 번만 UI 업데이트
        self.canvas_widget.canvas.ai_points = {'intersection': [], 'curve': [], 'endpoint': [], 'delete': []}
        self.canvas_widget.canvas.draw_points_layer()  # 점 레이어만 업데이트
        self.canvas_widget.canvas.scene().update()     # 화면 새로고침
        self.update_point_count_table()                # 점 개수 테이블 업데이트
        
        logger.info(f"🎉 일괄 삭제 완료: {ai_delete_count}개 점 제거, UI 업데이트 완료")
        
        # 🆕 편집 상태 캐시 업데이트
        self.save_current_edit_state()
        
        # 거리 계산 및 통계 업데이트 (항상 새로 실행)
        self.calculate_and_display_distances()
        self.update_modification_stats()
    ...
    def filter_points_by_district_boundary(self):
        """지구계 30m 반경 외부의 점들 삭제"""
        if self.file_mode != 'district' or not self.current_file:
            return
        ...
        try:
            ...
            # 지구계 폴리곤 로드
            logger.info("지구계 파일 로드 시작...")
            district_gdf = gpd.read_file(self.current_file)
            logger.info(f"원본 지구계 좌표계: {district_gdf.crs}")
            
            target_crs = self.get_target_crs()
            logger.info(f"목표 좌표계: {target_crs}")
            
            # 좌표계 처리
            if district_gdf.crs is None:
                # 좌표계 정보가 없으면 목표 좌표계로 설정
                logger.warning(f"지구계 파일에 좌표계 정보가 없습니다. {target_crs}로 설정합니다.")
                district_gdf = district_gdf.set_crs(target_crs)
                logger.info(f"좌표계 설정 완료: {district_gdf.crs}")
            elif district_gdf.crs != target_crs:
                # 좌표계가 다르면 변환
                district_gdf = district_gdf.to_crs(target_crs)
                logger.info(f"좌표계 변환 완료: {district_gdf.crs}")
            else:
                logger.info(f"좌표계 일치: {district_gdf.crs}")
            
            # 지구계 경계에서 30m 버퍼 생성
            logger.info("30m 버퍼 생성 중...")
            district_buffer = district_gdf.geometry.buffer(30.0).unary_union
            logger.info("30m 버퍼 생성 완료")
            
            # AI 점들 필터링
            logger.info("AI 점 필터링 시작...")
            if hasattr(self.canvas_widget.canvas, 'points'):
                for point_type in ['intersection', 'curve', 'endpoint']:
                    if point_type in self.canvas_widget.canvas.points:
                        points_list = self.canvas_widget.canvas.points[point_type]
                        
                        if points_list:  # 점이 있을 때만 처리
                            try:
                                logger.info(f"{point_type} 점 {len(points_list)}개 처리 중...")
                                # 모든 AI 점을 한 번에 GeoDataFrame으로 변환
                                ai_geometries = [Point(float(point[0]), float(point[1])) for point in points_list]
                                ai_gdf = gpd.GeoDataFrame(
                                    {'original_index': range(len(points_list))}, 
                                    geometry=ai_geometries, 
                                    crs=target_crs
                                )
                                
                                # 지구계 버퍼 내에 포함되는 점들만 필터링
                                within_buffer = ai_gdf.geometry.within(district_buffer) | ai_gdf.geometry.intersects(district_buffer)
                                filtered_indices = ai_gdf[within_buffer]['original_index'].tolist()
                                
                                filtered_points = [points_list[i] for i in filtered_indices]
                                self.canvas_widget.canvas.points[point_type] = filtered_points
                                logger.info(f"{point_type} 점 필터링: {len(points_list)} → {len(filtered_points)}개")
                            except Exception as e:
                                logger.error(f"AI {point_type} 점 필터링 오류: {e}")
                                # 오류 발생 시 원본 점들 유지
                                continue
            
            # 보존 기준점들 필터링
            logger.info("보존 기준점 필터링 시작...")
            if hasattr(self, 'reference_points') and self.reference_points:
                try:
                    logger.info(f"보존 기준점 {len(self.reference_points)}개 처리 중...")
                    # 모든 보존 기준점을 한 번에 GeoDataFrame으로 변환
                    ref_geometries = [Point(float(ref['x']), float(ref['y'])) for ref in self.reference_points]
                    ref_gdf = gpd.GeoDataFrame(
                        {'original_index': range(len(self.reference_points))}, 
                        geometry=ref_geometries, 
                        crs=target_crs
                    )
                    
                    # 지구계 버퍼 내에 포함되는 점들만 필터링
                    within_buffer = ref_gdf.geometry.within(district_buffer) | ref_gdf.geometry.intersects(district_buffer)
                    filtered_indices = ref_gdf[within_buffer]['original_index'].tolist()
                    
                    filtered_reference_points = [self.reference_points[i] for i in filtered_indices]
                    original_count = len(self.reference_points)
                    self.reference_points = filtered_reference_points
                    logger.info(f"보존 기준점 필터링: {original_count} → {len(filtered_reference_points)}개")
                except Exception as e:
                    logger.error(f"보존 기준점 필터링 오류: {e}")
                    # 오류 발생 시 원본 점들 유지
            
            # 보존 기준점 표시용 데이터도 필터링
            logger.info("보존 기준점 표시용 데이터 필터링 시작...")
            if hasattr(self.canvas_widget.canvas, 'reference_points_display'):
                if self.canvas_widget.canvas.reference_points_display:
                    try:
                        logger.info(f"표시용 보존 기준점 {len(self.canvas_widget.canvas.reference_points_display)}개 처리 중...")
                        # 모든 표시용 점을 한 번에 GeoDataFrame으로 변환
                        display_geometries = [Point(float(ref['point'][0]), float(ref['point'][1])) 
                                            for ref in self.canvas_widget.canvas.reference_points_display]
                        display_gdf = gpd.GeoDataFrame(
                            {'original_index': range(len(self.canvas_widget.canvas.reference_points_display))}, 
                            geometry=display_geometries, 
                            crs=target_crs
                        )
                        
                        # 지구계 버퍼 내에 포함되는 점들만 필터링
                        within_buffer_display = display_gdf.geometry.within(district_buffer) | display_gdf.geometry.intersects(district_buffer)
                        filtered_display_indices = display_gdf[within_buffer_display]['original_index'].tolist()
                        
                        filtered_display = [self.canvas_widget.canvas.reference_points_display[i] for i in filtered_display_indices]
                        original_display_count = len(self.canvas_widget.canvas.reference_points_display)
                        self.canvas_widget.canvas.reference_points_display = filtered_display
                        logger.info(f"표시용 보존 기준점 필터링: {original_display_count} → {len(filtered_display)}개")
                    except Exception as e:
                        logger.error(f"표시용 보존 기준점 필터링 오류: {e}")
                        # 오류 발생 시 원본 점들 유지
            ...
            logger.info("✅ 지구계 30m 반경 외부 점들 삭제 완료")
            
        except Exception as e:
            logger.error(f"지구계 경계 필터링 오류: {e}")
            QMessageBox.warning(self, "경고", f"지구계 경계 필터링 실패:\n{str(e)}")

    def perform_overlap_analysis(self):
        """보존 기준점 표시 및 AI/사용자 점들에 참조 속성 추가"""
        if not hasattr(self, 'reference_points') or not self.reference_points:
            return
            
        import math
        
        # AI 기준점과 사용자 추가 점 수집 (속성 추가용)
        ai_points = []
        if hasattr(self.canvas_widget.canvas, 'points'):
            for point_type in ['intersection', 'curve', 'endpoint']:
                points_list = self.canvas_widget.canvas.points.get(point_type, [])
        ...
        # 사용자 추가 점들도 포함
        if hasattr(self.canvas_widget.canvas, 'user_added_points'):
            for point in self.canvas_widget.canvas.user_added_points:
                ai_points.append({
                    'x': point[0],
                    'y': point[1],
                    'type': 'user',
        ...
        # Canvas의 point_attributes 초기화
        if not hasattr(self.canvas_widget.canvas, 'point_attributes'):
            self.canvas_widget.canvas.point_attributes = {}
        
        # 각 보존 기준점에 대해 20m 반경 내 AI/사용자 점 찾기 (속성 추가용)
        for ref_data in self.reference_points:
            ref_x = ref_data['x']
            ref_y = ref_data['y']
            
            # 20m 반경 내의 모든 AI/사용자 점에 속성 추가
            for ai_point in ai_points:
                distance = math.sqrt((ai_point['x'] - ref_x)**2 + (ai_point['y'] - ref_y)**2)
                if distance <= 20.0:
                    point_key = (ai_point['x'], ai_point['y'])
                    
                    # 이미 속성이 있으면 더 가까운 보존점 정보로 업데이트
                    if point_key in self.canvas_widget.canvas.point_attributes:
                        if self.canvas_widget.canvas.point_attributes[point_key].get('distance', float('inf')) > distance:
                            continue
                    
                    # 점 유형에 따른 메시지
                    if ai_point['type'] == 'user':
                        attribute_text = "보존기준점 참고"
                    else:
                        attribute_text = "보존기준점 근접"
                    
                    self.canvas_widget.canvas.point_attributes[point_key] = {
                        'reference': attribute_text,
                        'distance': round(distance, 2)
                    }
        
        # Canvas에 보존 기준점들 표시 (모두 회색으로)
        if not hasattr(self.canvas_widget.canvas, 'reference_points_display'):
            self.canvas_widget.canvas.reference_points_display = []
            
        self.canvas_widget.canvas.reference_points_display = []
        
        # 모든 보존 점들을 회색으로 표시 (참고용)
        for ref_data in self.reference_points:
            self.canvas_widget.canvas.reference_points_display.append({
                'point': (ref_data['x'], ref_data['y']),
                'color': 'gray',  # 회색으로 변경
                'type': 'reference'
            })
        
        # Canvas에 보존 기준점 표시 활성화
        if hasattr(self.canvas_widget.canvas, 'show_reference_points'):
            self.canvas_widget.canvas.show_reference_points = True
        ...
        attrs_added = len(self.canvas_widget.canvas.point_attributes)
        logger.info(f"📍 보존 기준점 {total_refs}개 표시 완료 (회색), {attrs_added}개 점에 참조 속성 추가")
        
        return {'reference_points': total_refs, 'attributes_added': attrs_added}

    def upload_reference_points(self):
        """보존 지적기준점 파일에서 좌표 불러오기 (Excel/SHP 지원)"""
        file_path, _ = QFileDialog.getOpenFileName(
            self, "보존 지적기준점 파일 선택", "", 
            "지원 파일 (*.xlsx *.xls *.shp);;Excel (*.xlsx *.xls);;Shapefile (*.shp)")
        ...
        if file_path:
            try:
                ...
                if file_ext in ['.xlsx', '.xls']:
                    ...
                    # Point geometry에서 좌표 추출
                    for idx, row in gdf.iterrows():
                        geom = row.geometry
                        
                        if geom and hasattr(geom, 'x') and hasattr(geom, 'y'):
                            all_points.append({'x': geom.x, 'y': geom.y})
                        elif geom and hasattr(geom, 'coords'):
                            # LineString이나 Polygon의 경우 첫 번째 좌표만 사용
                            coords = list(geom.coords)
                            if coords:
                                all_points.append({'x': coords[0][0], 'y': coords[0][1]})
                else:
                ...
                unique_points = {}
                for point_data in all_points:
                    coord_key = (point_data['x'], point_data['y'])
                    if coord_key not in unique_points:
                        unique_points[coord_key] = point_data
                
                self.reference_points = list(unique_points.values())
                original_count = len(all_points)
                unique_count = len(self.reference_points)
                
                file_type = "Excel" if file_ext in ['.xlsx', '.xls'] else "Shapefile"
                if original_count != unique_count:
                    duplicates_removed = original_count - unique_count
                    logger.info(f"📍 {file_type} 중복 제거: 원본 {original_count}개 → 유효 {unique_count}개 (중복 제거: {duplicates_removed}개)")
                else:
                    logger.info(f"📍 {file_type} 중복 없음: {unique_count}개 좌표 로드")
                
                # Canvas에 전달 - 기본 보존 기준점으로 먼저 표시
                self.canvas_widget.canvas.reference_points = self.reference_points
                
                # 지구계 30m 반경 외부 보존 기준점 필터링
                self.filter_points_by_district_boundary()
                
                # AI 점이 없는 경우 바로 보존점만 표시
                if not hasattr(self.canvas_widget.canvas, 'points') or not any(len(pts) for pts in self.canvas_widget.canvas.points.values()):
                    # AI 점이 없으면 모든 보존점을 검은색으로 표시
                    if not hasattr(self.canvas_widget.canvas, 'reference_points_display'):
                        self.canvas_widget.canvas.reference_points_display = []
                    ...
                    # AI 점이 있으면 보존점 표시 및 속성 추가
                    self.perform_overlap_analysis()
                ...
                if original_count != unique_count:
                    ...
                    QMessageBox.information(
                        self, "완료", 
                        f"{file_type}에서 {unique_count}개 보존 지적기준점을 불러왔습니다.\n"
                        f"(원본: {original_count}개, 중복 제거: {duplicates_removed}개)\n\n"
                        f"참고용으로 회색 다이아몬드로 표시됩니다."
                    )
                else:
                    QMessageBox.information(
                        self, "완료", 
                ...
                logger.error(f"보존 지적기준점 파일 로드 오류: {e}")
                QMessageBox.critical(self, "오류", f"보존 지적기준점 파일 로드 실패:\n{str(e)}")

    def update_point_count_table(self):
        """점 개수 비교 테이블 업데이트 (개선된 버전)"""
        # 작은 폰트 설정
        small_font = QFont()
        small_font.setPointSize(8)
        
        # 1. AI 기준점 (고정값 - 처음 AI가 검출한 개수)
        ai_item = QTableWidgetItem(str(self.initial_ai_count))
        ai_item.setFont(small_font)
        self.point_count_table.setItem(0, 1, ai_item)
        
        # 2. 보존 지적기준점 개수
        reference_count = len(self.reference_points)
        reference_item = QTableWidgetItem(str(reference_count))
        reference_item.setFont(small_font)
        self.point_count_table.setItem(1, 1, reference_item)
        
        # 3. 선점 기준점 (AI 점 + 사용자 추가 점만, 보존기준점 제외)
        current_count = 0
        if hasattr(self.canvas_widget.canvas, 'points'):
            for point_type in ['intersection', 'curve', 'endpoint']:
                current_count += len(self.canvas_widget.canvas.points.get(point_type, []))
        
        # 사용자가 추가한 점도 포함
        if hasattr(self.canvas_widget.canvas, 'user_added_points'):
            current_count += len(self.canvas_widget.canvas.user_added_points)
        
        # 보존 기준점은 제외 (참고용일 뿐)
            
        final_item = QTableWidgetItem(str(current_count))
        final_item.setFont(small_font)
        final_item.setBackground(QColor(144, 238, 144))  # 연한 녹색
        self.point_count_table.setItem(2, 1, final_item)
        
        # 4. 중복 (보존점과 근접한 AI/사용자 점 개수)
        overlap_count = 0
        if hasattr(self.canvas_widget.canvas, 'point_attributes'):
            overlap_count = len(self.canvas_widget.canvas.point_attributes)
        overlap_item = QTableWidgetItem(f"{overlap_count}점")
        overlap_item.setFont(small_font)
        self.point_count_table.setItem(3, 1, overlap_item)
        
        # 5. 삭제된 점 개수
        deleted_count = 0
        if hasattr(self.canvas_widget.canvas, 'deleted_points_count'):
            deleted_count = self.canvas_widget.canvas.deleted_points_count
        deleted_item = QTableWidgetItem(f"{deleted_count}점")
        deleted_item.setFont(small_font)
        self.point_count_table.setItem(4, 1, deleted_item)
        
        # 6. 추가된 점 개수
        added_count = 0
        if hasattr(self.canvas_widget.canvas, 'user_added_points'):
            added_count = len(self.canvas_widget.canvas.user_added_points)
        added_item = QTableWidgetItem(f"{added_count}점")
        added_item.setFont(small_font)
        self.point_count_table.setItem(5, 1, added_item)
            
        # 상단 최종 개수 라벨 업데이트
        self.final_count_label.setText(f"🎯 최종 선점 기준점: {current_count}개")
    
    def on_point_changed(self, category, x, y):
        """Canvas에서 점이 변경될 때 호출되는 콜백 - 더 이상 사용하지 않음"""
        # Lambda 함수로 대체됨
        pass
        
        # 🆕 편집 상태 자동 캐시 업데이트 (거리 계산 제외)
        self.save_current_edit_state()
        
        logger.debug(f"점 변경 감지: {category} at ({x:.1f}, {y:.1f}) - 테이블 업데이트 및 캐시 저장 완료")
    ...
    def export_points_to_shp(self):
        """최종 점들+클립된도로망+지구계를 별도 SHP 파일들로 저장"""
        try:
            # 입력 파일명 기준으로 출력 파일명 생성
            if not self.current_file:
                QMessageBox.warning(self, "경고", "입력 파일이 없습니다.")
                return
                
            base_name = Path(self.current_file).stem
            output_dir = QFileDialog.getExistingDirectory(self, "저장 폴더 선택")
            if not output_dir:
                return
            ...
            # 좌표계 설정
            crs = self.get_target_crs() if self.file_mode == 'district' else 'EPSG:5186'
            
            # 1. 최종 점들 저장 (AI + 보존 + 사용자편집)
            all_points = []
            
            # AI 점들 (중첩분석 후 남은 것들)
            if hasattr(self.canvas_widget.canvas, 'points'):
                for category, point_list in self.canvas_widget.canvas.points.items():
                    for point in point_list:
                        ...
                        # 속성 정보 추가 (보존 기준점 참조 등)
                        point_key = (float(point[0]), float(point[1]))
                        if hasattr(self.canvas_widget.canvas, 'point_attributes'):
                            if point_key in self.canvas_widget.canvas.point_attributes:
                                attrs = self.canvas_widget.canvas.point_attributes[point_key]
                                point_data['비고'] = attrs.get('reference', '')
                                point_data['거리'] = attrs.get('distance', 0)
                ...
                for point in self.canvas_widget.canvas.user_added_points:
                    ...
                    if hasattr(self.canvas_widget.canvas, 'point_attributes'):
                        if point_key in self.canvas_widget.canvas.point_attributes:
                            attrs = self.canvas_widget.canvas.point_attributes[point_key]
                            point_data['비고'] = attrs.get('reference', '')
                            point_data['참조ID'] = attrs.get('ref_id', '')
                            point_data['거리'] = attrs.get('distance', 0)
                    
                    all_points.append(point_data)
            
            # 보존 지적기준점은 내보내지 않음 (참고용일 뿐)
            
            if all_points:
                points_gdf = gpd.GeoDataFrame(all_points, crs=crs)
                points_file = output_dir / f"{base_name}_points.shp"
                points_gdf.to_file(points_file, encoding='utf-8')
                saved_files.append(f"점 데이터: {points_file.name} ({len(all_points)}개)")
            
            # 2. 클립된 도로망 저장 (화면에 띄워진 클립된 도로망)
            try:
                road_data = []
                
                # Canvas에서 클립된 도로망 데이터 가져오기 (road_geometry)
                if hasattr(self.canvas_widget.canvas, 'road_geometry') and self.canvas_widget.canvas.road_geometry:
                    logger.info("클립된 도로망 데이터 처리 중...")
                    
                    # road_geometry가 리스트인 경우 (여러 geometry 객체)
                    if isinstance(self.canvas_widget.canvas.road_geometry, list):
                        for i, geom in enumerate(self.canvas_widget.canvas.road_geometry):
                            if geom and hasattr(geom, 'geom_type'):
                                road_data.append({
                                    'geometry': geom,
                                    'road_id': i + 1,
                    ...
                    # road_geometry가 단일 geometry인 경우
                    elif hasattr(self.canvas_widget.canvas.road_geometry, 'geom_type'):
                        geom = self.canvas_widget.canvas.road_geometry
                        road_data.append({
                            'geometry': geom,
                            'road_id': 1,
                            'geom_type': geom.geom_type,
                    ...
                    # MultiGeometry인 경우 (geoms 속성 있음)
                    elif hasattr(self.canvas_widget.canvas.road_geometry, 'geoms'):
                        for i, geom in enumerate(self.canvas_widget.canvas.road_geometry.geoms):
                            road_data.append({
                                'geometry': geom,
                                'road_id': i + 1,
                                'geom_type': geom.geom_type,
                ...
                if road_data:
                    # Geometry 타입별로 분리하여 저장
                    from shapely.geometry import GeometryCollection, LineString, Polygon, MultiPolygon, MultiLineString
                    ...
                    for item in road_data:
                        ...
                        # GeometryCollection 처리 - 내부 geometry들을 개별적으로 처리
                            ...
                            for sub_geom in geom.geoms:
                                if isinstance(sub_geom, (LineString, MultiLineString)):
                                    line_data.append({
                                        ...
                                        'geom_type': sub_geom.geom_type,
                                        'length': sub_geom.length if hasattr(sub_geom, 'length') else 0
                                    })
                                elif isinstance(sub_geom, (Polygon, MultiPolygon)):
                                    polygon_data.append({
                                        'geometry': sub_geom,
                                        'road_id': item['road_id'],
                                        'geom_type': sub_geom.geom_type,
                                        'area': sub_geom.area if hasattr(sub_geom, 'area') else 0
                                    })
                        # LineString 타입
                        ...
                        # Polygon 타입
                            ...
                            polygon_item = item.copy()
                            polygon_item['area'] = geom.area if hasattr(geom, 'area') else 0
                            polygon_data.append(polygon_item)
                        else:
                            logger.warning(f"알 수 없는 geometry 타입: {type(geom)}")
                    
                    # LineString 데이터가 있으면 저장
                    if line_data:
                        lines_gdf = gpd.GeoDataFrame(line_data, crs=crs)
                        roads_file = output_dir / f"{base_name}_roads.shp"
                        lines_gdf.to_file(roads_file, encoding='utf-8')
                        saved_files.append(f"도로망 (선): {roads_file.name} ({len(line_data)}개)")
                        logger.info(f"클립된 도로망 (선) 저장 완료: {len(line_data)}개")
                    
                    # Polygon 데이터가 있으면 별도로 저장
                    if polygon_data:
                        polygons_gdf = gpd.GeoDataFrame(polygon_data, crs=crs)
                        polygon_file = output_dir / f"{base_name}_road_polygons.shp"
                        polygons_gdf.to_file(polygon_file, encoding='utf-8')
                        saved_files.append(f"도로망 (폴리곤): {polygon_file.name} ({len(polygon_data)}개)")
                        logger.info(f"클립된 도로망 (폴리곤) 저장 완료: {len(polygon_data)}개")
                    
                    if not line_data and not polygon_data:
                        logger.warning("유효한 도로망 데이터가 없습니다")
                else:
                    logger.warning("화면에 표시된 클립된 도로망 데이터가 없습니다")
                    
            except Exception as e:
                logger.error(f"클립된 도로망 저장 실패: {e}")
                import traceback
                traceback.print_exc()
            
            # 3. 지구계 폴리곤 저장 (지구계 모드인 경우)
            if self.file_mode == 'district' and self.current_file:
                try:
                    logger.info("지구계 폴리곤 저장 중...")
                    district_gdf = gpd.read_file(self.current_file)
                    
                    # 좌표계 처리
                    if district_gdf.crs is None:
                        district_gdf = district_gdf.set_crs(crs)
                    elif district_gdf.crs != crs:
                        district_gdf = district_gdf.to_crs(crs)
                    
                    district_file = output_dir / f"{base_name}_district.shp"
                    district_gdf.to_file(district_file, encoding='utf-8')
                    saved_files.append(f"지구계: {district_file.name} ({len(district_gdf)}개)")
                    logger.info(f"지구계 폴리곤 저장 완료: {len(district_gdf)}개")
                except Exception as e:
                    logger.error(f"지구계 저장 실패: {e}")
                    import traceback
                    traceback.print_exc()
    ...
    def export_all_to_gpkg(self):
        """최종 점들+클립된도로망+지구계를 GPKG 파일로 저장 (단일 파일, 다중 레이어)"""
        try:
            # 입력 파일명 기준으로 출력 파일명 생성
            if not self.current_file:
                QMessageBox.warning(self, "경고", "입력 파일이 없습니다.")
            ...
            base_name = Path(self.current_file).stem
            default_filename = f"{base_name}_complete.gpkg"
            
            # 파일 저장 다이얼로그
            file_path, _ = QFileDialog.getSaveFileName(
                self, "전체 결과 GPKG 파일 저장", default_filename, "GeoPackage (*.gpkg)"
            )
            ...
            if all_points:
                ...
                saved_layers.append(f"points: {len(all_points)}개")
            
            # 2. 클립된 도로망 레이어 (화면에 띄워진 클립된 도로망)
            try:
                road_data = []
                ...
                if road_data:
                    # Geometry 타입별로 분리하여 저장 (GPKG는 여러 geometry 타입 지원하지만 안전하게 처리)
                    from shapely.geometry import GeometryCollection, LineString, Polygon, MultiPolygon, MultiLineString
                    
                    # 모든 geometry를 처리 (GeometryCollection 분해)
                    processed_data = []
                    ...
                    for item in road_data:
                        ...
                        if isinstance(geom, GeometryCollection):
                            ...
                            processed_item['area'] = geom.area if hasattr(geom, 'area') else 0
                            processed_data.append(processed_item)
                    
                    if processed_data:
                        roads_gdf = gpd.GeoDataFrame(processed_data, crs=crs)
                        roads_gdf.to_file(file_path, layer='roads', driver='GPKG')
                        saved_layers.append(f"roads: {len(processed_data)}개")
                        logger.info(f"클립된 도로망 레이어 저장 완료: {len(processed_data)}개")
                    else:
                        logger.warning("유효한 도로망 데이터가 없습니다")
                else:
                    logger.warning("화면에 표시된 클립된 도로망 데이터가 없습니다")
                ...
                logger.error(f"클립된 도로망 레이어 저장 실패: {e}")
                import traceback
                traceback.print_exc()
            
            # 3. 지구계 폴리곤 레이어 (지구계 모드인 경우)
            if self.file_mode == 'district' and self.current_file:
                try:
                    logger.info("지구계 폴리곤 레이어 처리 중...")
                    district_gdf = gpd.read_file(self.current_file)
                    
                    # 좌표계 처리 (30m 필터링과 동일한 로직)
                    if district_gdf.crs is None:
                        logger.warning(f"지구계 파일에 좌표계 정보가 없습니다. {crs}로 설정합니다.")
                        district_gdf = district_gdf.set_crs(crs)
                    elif district_gdf.crs != crs:
                        district_gdf = district_gdf.to_crs(crs)
                    
                    district_gdf.to_file(file_path, layer='district', driver='GPKG')
                    saved_layers.append(f"district: {len(district_gdf)}개")
                    logger.info(f"지구계 폴리곤 레이어 저장 완료: {len(district_gdf)}개")
                except Exception as e:
                    logger.error(f"지구계 레이어 저장 실패: {e}")
                    import traceback
                    traceback.print_exc()
    ...
    def auto_calculate_distances(self):
        ...
        try:
            logger.info("🚀🚀🚀 auto_calculate_distances 호출됨! 🚀🚀🚀")
            
            # 현재 상태 확인
            points_count = 0
            if hasattr(self, 'canvas_widget') and hasattr(self.canvas_widget, 'canvas'):
                points_count = sum(len(self.canvas_widget.canvas.points.get(cat, [])) for cat in ['intersection', 'curve', 'endpoint'])
                logger.info(f"🔍 현재 점 개수: {points_count}")
            else:
                logger.warning("❌ canvas_widget 또는 canvas가 없음")
                return
    ...
    def manual_calculate_distances(self):
        """🔗 수동 거리 계산 버튼 클릭 시 호출"""
        try:
            logger.info("🔗 수동 거리 계산 버튼 클릭됨")
            
            # 1. 기존 거리 표시 지우기 🧹
            if hasattr(self.canvas_widget.canvas, 'distance_connections'):
                self.canvas_widget.canvas.distance_connections = []
                logger.info("🧹 기존 거리 표시 지움")
            
            if hasattr(self.canvas_widget.canvas, 'distance_items'):
                for item in self.canvas_widget.canvas.distance_items:
                    try:
                        if item is not None and hasattr(item, 'scene') and item.scene() is not None:
                            item.scene().removeItem(item)  # scene()를 통해 scene 객체에 접근
                    except (RuntimeError, AttributeError):
                        pass
                self.canvas_widget.canvas.distance_items = []
                logger.info("🧹 기존 거리 그래픽 아이템 지움")
            ...
            # 2. 현재 상태 확인
            points_count = sum(len(self.canvas_widget.canvas.points.get(cat, [])) for cat in ['intersection', 'curve', 'endpoint'])
            road_available = hasattr(self.canvas_widget, 'road_gdf') and self.canvas_widget.road_gdf is not None
            
            logger.info(f"🔍 현재 상태 - 점 개수: {points_count}, 도로 데이터: {road_available}")
            
            if points_count == 0:
                self.distance_label.setText("점간 거리: 먼저 AI 분석을 실행하세요")
                return
            
            if not road_available:
                self.distance_label.setText("점간 거리: 도로 데이터가 없습니다")
                return
            
            # 3. 새로운 거리 계산 실행
            self.calculate_smart_distance_connections()
            
            logger.info("✅ 수동 거리 계산 완료")
            
        except Exception as e:
            logger.error(f"수동 거리 계산 실패: {e}")
            import traceback
            logger.error(f"상세 오류: {traceback.format_exc()}")
            self.distance_label.setText("점간 거리: 수동 계산 실패")
    
    # recalculate_distances 메서드 제거됨 - 자동 거리 계산으로 대체
    
    def calculate_smart_distance_connections(self):
        """도로 폴리곤 기반 거리 연결 로직 - 성공 검증된 방식"""
        logger.info("🚀 calculate_smart_distance_connections 시작")
        
        # 1. 기본 데이터 검증
        if not self.canvas_widget.canvas.points:
            logger.warning("❌ canvas.points가 없음")
            self.distance_label.setText("점간 거리: 분석할 점이 없습니다")
            return
        
        logger.info(f"📍 canvas.points 내용: {dict(self.canvas_widget.canvas.points)}")
        
        # 2. 모든 점 수집
        all_points = []
        for category in ['intersection', 'curve', 'endpoint']:
            category_points = self.canvas_widget.canvas.points.get(category, [])
            logger.info(f"📍 {category} 점 개수: {len(category_points)}")
        ...
        logger.info(f"📍 전체 수집된 점 개수: {len(all_points)}")
        
        if len(all_points) < 2:
            logger.warning(f"❌ 점이 부족함: {len(all_points)}개 (최소 2개 필요)")
            self.distance_label.setText("점간 거리: 점이 부족합니다 (최소 2개 필요)")
            return
        
        # 3. 도로 폴리곤 데이터 확인
        road_gdf = getattr(self.canvas_widget, 'road_gdf', None)
        logger.info(f"🛣️ road_gdf 확인 - 존재: {road_gdf is not None}, 비어있지않음: {road_gdf is not None and not road_gdf.empty}")
        
        if road_gdf is None or road_gdf.empty:
            logger.warning("❌ 도로망 데이터가 없음")
            self.distance_label.setText("점간 거리: 도로망 데이터가 없습니다")
            return
        
        logger.info(f"🛣️ 도로 폴리곤 개수: {len(road_gdf)}")
        
        # 4. 도로 폴리곤 기반 거리 연결 찾기 (5m 버퍼)
        valid_connections = self.find_road_polygon_connections(all_points, road_gdf)
        ...
        logger.info(f"🔗 도로 폴리곤 기반 거리 연결 완료: {len(valid_connections)}개 연결")
    
    def find_smart_distance_connections(self, all_points, skeleton, road_union):
        """도로 폴리곤 기반 직접 연결 찾기 - 성공 검증된 방식으로 교체"""
        # 기존 스켈레톤 기반 로직을 도로 폴리곤 기반으로 교체
        # road_union 대신 개별 도로 폴리곤 사용
        try:
            road_gdf = getattr(self.canvas_widget, 'road_gdf', None)
            if road_gdf is None or road_gdf.empty:
                logger.warning("도로 GDF가 없어서 기본 연결 방식 사용")
                return []
            
            return self.find_road_polygon_connections(all_points, road_gdf)
        except Exception as e:
            logger.error(f"도로 폴리곤 기반 연결 실패: {e}")
            return []
    
    def find_road_polygon_connections(self, all_points, road_gdf):
        """도로 폴리곤 기반 직접 연결 찾기 - 성공 검증된 방식"""
        from shapely.geometry import LineString, Point
        import numpy as np
        
        logger.info(f"🔍 도로 폴리곤 연결 검사 시작 - 점 개수: {len(all_points)}, 도로 폴리곤 개수: {len(road_gdf)}")
        ...
        for i, point1_data in enumerate(all_points):
            for j, point2_data in enumerate(all_points[i+1:], i+1):
                ...
                # 2. 직선이 도로망(5m 버퍼) 내에 있는지 확인
                line_in_road = self.is_line_in_roads(line, road_gdf, buffer_distance=3.0)
                
                # 3. 직선 상에 다른 점이 있는지 확인
                has_intermediate_point = False
                for k, other_point_data in enumerate(all_points):
                    if k in [i, j]:  # 시작점, 끝점은 제외
                        continue
                    
                    # 점과 직선 사이의 거리 계산
                    point = Point(other_point_data['coord'])
                ...
                if line_in_road and not has_intermediate_point:
                    ...
                    logger.info(f"✅ 유효한 연결 발견: P1({point1[0]:.1f},{point1[1]:.1f}) ↔ P2({point2[0]:.1f},{point2[1]:.1f}) = {euclidean_distance:.1f}m")
        
        logger.info(f"🔍 도로 폴리곤 연결 검사 완료 - 총 {total_pairs}개 쌍 검사, {len(valid_connections)}개 유효 연결 발견")
        return valid_connections
    
    def is_line_in_roads(self, line, road_gdf, buffer_distance=3.0):
    ...
    def update_canvas_distance_display(self, valid_connections):
        ...
        try:
            ...
            logger.info(f"🔧 Canvas 전달 전 - 연결 개수: {len(display_connections)}")
            ...
            if hasattr(self.canvas_widget.canvas, 'distance_connections'):
                ...
                logger.info(f"🔧 Canvas에 설정됨 - distance_connections 개수: {len(self.canvas_widget.canvas.distance_connections)}")
            else:
                # distance_connections 속성이 없으면 생성
                self.canvas_widget.canvas.distance_connections = display_connections
                self.canvas_widget.canvas.show_distance_connections = True
                logger.info(f"🔧 Canvas에 새로 생성됨 - distance_connections 개수: {len(self.canvas_widget.canvas.distance_connections)}")
            
            # 전체 Canvas 업데이트
            self.canvas_widget.canvas.update_display()
            
            # 강제로 거리선 그리기 (체크박스 상태 확인)
            if hasattr(self.canvas_widget.canvas, 'draw_distance_lines'):
                # 거리표시 체크박스가 체크되어 있을 때만 그리기
                if hasattr(self, 'show_distance_checkbox') and self.show_distance_checkbox.isChecked():
                    self.canvas_widget.canvas.draw_distance_lines()
                    logger.info("🔧 강제로 draw_distance_lines 호출 완료")
                else:
                    logger.info("🔧 거리표시 체크박스가 해제되어 있어 거리선 그리기 생략")
    ...
    def try_load_cached_edit_state(self):
        """캐시된 편집 상태 로드 시도"""
        try:
            if not self.current_file:
                return None
            ...
            polygon_index = getattr(self, 'current_polygon_index', 0)
            cached_state = cache.load_polygon_edit_state(self.current_file, polygon_index)
            
            if cached_state:
                logger.info(f"✅ 캐시된 편집 상태 발견: {Path(self.current_file).name} (폴리곤 {polygon_index + 1})")
                return cached_state
            else:
                logger.info(f"📭 캐시된 편집 상태 없음: {Path(self.current_file).name} (폴리곤 {polygon_index + 1})")
                return None
                
        except Exception as e:
            logger.warning(f"편집 상태 캐시 로드 실패: {e}")
            return None
    
    def apply_cached_edit_state(self, cached_state):
        """캐시된 편집 상태 적용"""
        try:
            # 스켈레톤 데이터 복원
            if 'skeleton' in cached_state:
                self.canvas_widget.skeleton = cached_state['skeleton']
                self.canvas_widget.canvas.skeleton = cached_state['skeleton']
            
            # 사용자 편집 점들 복원
            if 'points' in cached_state:
                self.canvas_widget.canvas.points = cached_state['points']
            
            # AI 예측 결과 복원 (있는 경우)
            if 'ai_points' in cached_state:
                self.canvas_widget.canvas.ai_points = cached_state['ai_points']
            
            # 도로망 데이터 복원 (있는 경우)
            if 'road_gdf' in cached_state and cached_state['road_gdf'] is not None:
                self.canvas_widget.set_road_data(cached_state['road_gdf'])
            
            # AI 초기 개수 설정 (캐시 복원 시)
            if self.initial_ai_count == 0 and 'points' in cached_state:
                self.initial_ai_count = sum(len(cached_state['points'].get(cat, [])) for cat in ['intersection', 'curve', 'endpoint'])
                logger.info(f"AI 초기 개수 저장 (캐시 복원): {self.initial_ai_count}개")
            ...
            # 결과 표시
            total_points = sum(len(self.canvas_widget.canvas.points.get(cat, [])) 
                             for cat in ['intersection', 'curve', 'endpoint'])
            self.result_label.setText(f"처리 완료: {total_points}개 점")
            
            logger.info(f"✅ 분석석 완료: {total_points}개 점")
            
        except Exception as e:
            logger.error(f"편집 상태 적용 실패: {e}")
    
    def save_current_edit_state(self):
        """현재 편집 상태 캐시 저장"""
        try:
            if not self.current_file or not hasattr(self.canvas_widget, 'skeleton'):
                return
            ...
            edit_state = {
                'skeleton': self.canvas_widget.skeleton,
                'points': self.canvas_widget.canvas.points.copy() if hasattr(self.canvas_widget.canvas, 'points') else {},
                'ai_points': self.canvas_widget.canvas.ai_points.copy() if hasattr(self.canvas_widget.canvas, 'ai_points') else {},
                'road_gdf': getattr(self.canvas_widget, 'road_gdf', None),
                'file_mode': self.file_mode,
                'target_crs': self.get_target_crs() if self.file_mode == 'district' else None
            }
            ...
            # 캐시 저장 (거리 계산 데이터는 제외)
            cache.save_polygon_edit_state(self.current_file, polygon_index, edit_state)
            
            logger.debug(f"💾 편집 상태 캐시 저장: {Path(self.current_file).name} (폴리곤 {polygon_index + 1})")
            
        except Exception as e:
            logger.warning(f"편집 상태 캐시 저장 실패: {e}")
