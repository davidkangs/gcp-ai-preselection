...
class InferenceTool(QMainWindow):
    ...
    def process_road_file(self, file_path):
        ...
        try:
            ...
            if result['success']:
                ...
                if result.get('stats'):
                    stats = result['stats']
                    total_detected = (stats.get('detected_intersections', 0) + 
                                    stats.get('detected_curves', 0) + 
                                    stats.get('detected_endpoints', 0))
                    self.result_label.setText(f"최종 {total_detected}개 점")
    ...
    def load_polygon_result(self, result):
        ...
        if 'polygon_results' in result:
            ...
            if 'skeleton' in polygon_result:
                ...
                # 🚀 v1 방식: 파이프라인 결과는 이미 처리된 데이터이므로 가벼운 처리만
                logger.info("✅ 파이프라인 매니저 결과 - 간단한 최적화만 수행")
                
                # 기본 중복점 제거만 수행 (무거운 연산 제외)
                removed_count = self.basic_duplicate_removal()
                
                # 점 개수 계산
                final_points = sum(len(self.canvas_widget.canvas.points.get(cat, [])) 
                ...
                info_text += f"파이프라인 처리 완료 ({removed_count}개 정리)"
                self.result_label.setText(f"처리 완료:\n{info_text}")
                self.statusBar().showMessage(f"지구계 파이프라인 완료 - 총 {final_points}개 점 ({removed_count}개 정리)")
    ...
    def run_multi_stage_optimization(self):
        ...
        try:
            ...
            logger.info(f"🔧 스마트 최적화 시작 - 스켈레톤: {skeleton_size}개 점")
            
            # 📊 데이터 크기에 따른 필터링 방식 자동 선택
            if skeleton_size <= 800:
                # 💎 작은 데이터: 정교한 필터링 (높은 정확도)
                logger.info(f"💎 작은 데이터셋({skeleton_size}점) - 정교한 필터링 모드")
                return self._run_precise_optimization()
            elif skeleton_size <= 2000:
                # ⚖️ 중간 데이터: 하이브리드 필터링 (균형)
                logger.info(f"⚖️ 중간 데이터셋({skeleton_size}점) - 하이브리드 필터링 모드")
                return self._run_hybrid_optimization()
            else:
                # ⚡ 큰 데이터: 빠른 필터링 (고성능)
                logger.info(f"⚡ 큰 데이터셋({skeleton_size}점) - 빠른 필터링 모드")
