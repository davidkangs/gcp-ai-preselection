...
class InferenceTool(QMainWindow):
    def __init__(self):
        ...
        self.hybrid_filter = create_hybrid_filter(
            dbscan_eps=20.0,      # 20m 클러스터링
            network_max_dist=50.0, # 50m 네트워크 연결
    ...
    def remove_duplicate_points(self):
        """하이브리드 필터를 사용한 중복점 제거 (DBSCAN + 네트워크 + 중요도 + 역할)"""
        ...
        try:
            ...
            for x, y in self.canvas_widget.canvas.points.get('intersection', []):
                ...
                point_roles[(x, y)] = 'intersection'
            
            for x, y in self.canvas_widget.canvas.points.get('curve', []):
                all_points.append((x, y))
                point_roles[(x, y)] = 'curve'
            
            for x, y in self.canvas_widget.canvas.points.get('endpoint', []):
                all_points.append((x, y))
                point_roles[(x, y)] = 'endpoint'
            
            if not all_points:
                QMessageBox.information(self, "완료", "필터링할 점이 없습니다.")
                return
            ...
            # 도로 폴리곤 생성 (스켈레톤 기반)
            road_polygon = self._create_road_polygon_from_skeleton(skeleton)
            
            # 하이브리드 필터링 실행
            filtered_points = self.hybrid_filter.hybrid_filter(
                points=all_points,
                skeleton=skeleton,
            ...
            filtered_counts = {
                'intersection': len(filtered_by_role['intersection']),
                'curve': len(filtered_by_role['curve']),
                'endpoint': len(filtered_by_role['endpoint'])
            }
            
            removed_counts = {
                'intersection': original_counts['intersection'] - filtered_counts['intersection'],
                'curve': original_counts['curve'] - filtered_counts['curve'],
                'endpoint': original_counts['endpoint'] - filtered_counts['endpoint']
            }
            ...
            QMessageBox.information(
                self, "하이브리드 필터링 완료",
                f"🎯 하이브리드 필터링 (DBSCAN + 네트워크 + 중요도 + 역할)\n\n"
                f"제거된 포인트:\n"
                ...
                f"남은 포인트:\n"
                f"- 교차점: {filtered_counts['intersection']}개\n"
                f"- 커브점: {filtered_counts['curve']}개\n"
                f"- 끝점: {filtered_counts['endpoint']}개"
            )
            
            self.update_modification_stats()
            
        except Exception as e:
            logger.error(f"하이브리드 필터링 오류: {e}")
            QMessageBox.critical(self, "오류", f"하이브리드 필터링 실패:\n{str(e)}")
    
    def _create_road_polygon_from_skeleton(self, skeleton):
        """스켈레톤에서 도로 폴리곤 생성"""
        try:
            ...
            # 도로 폴리곤 생성 (버퍼 적용)
            road_polygon = skeleton_line.buffer(15.0)  # 15m 버퍼
            
            return road_polygon
            
        except Exception as e:
            logger.warning(f"도로 폴리곤 생성 실패: {e}")
            return None
    ...
    def filter_overlapping_points(self):
        ...
        try:
            ...
            # 하이브리드 필터링 실행 (완화된 설정)
            strict_filter = create_hybrid_filter(
                dbscan_eps=25.0,      # 완화된 클러스터링
                network_max_dist=80.0, # 더 긴 네트워크 연결
                road_buffer=5.0       # 더 큰 도로 버퍼
            )
            
            # 필터링 전 점 개수 로그
            logger.info(f"🔍 필터링 전 점 개수: 교차점={len([p for p in all_points if point_roles.get(p) == 'intersection'])}개, "
                       f"커브={len([p for p in all_points if point_roles.get(p) == 'curve'])}개, "
                       f"끝점={len([p for p in all_points if point_roles.get(p) == 'endpoint'])}개")
            
            filtered_points = strict_filter.hybrid_filter(
                points=all_points,
                skeleton=skeleton,
                road_polygon=road_polygon,
                point_roles=point_roles
            ...
            # 필터링 후 점 개수 로그
            logger.info(f"✅ 필터링 후 점 개수: {len(filtered_points)}개 (제거: {len(all_points) - len(filtered_points)}개)")
            ...
            logger.info(f"하이브리드 중복 점 필터링 완료: {removed_count}개 점 제거")
            
        except Exception as e:
            logger.error(f"하이브리드 중복 점 필터링 오류: {e}")
