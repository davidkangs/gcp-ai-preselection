...
class InferenceTool(QMainWindow):
    ...
    def init_ui(self):
        ...
        splitter.setSizes([280, 1120])  # 20:80 비율로 좌측 패널 축소
    ...
    def create_left_panel(self):
        ...
        self.crs_5186_radio = QRadioButton("중부원점")
        ...
        self.crs_5187_radio = QRadioButton("동부원점")
        ...
        self.show_excel_checkbox = QCheckBox("실제기준점표시")
        ...
        # 🔗 수동 거리 계산 버튼 추가
        manual_distance_btn = QPushButton("🔗 거리 계산 실행")
        manual_distance_btn.clicked.connect(self.manual_calculate_distances)
        manual_distance_btn.setStyleSheet("QPushButton {background-color: #4CAF50; color: white; font-weight: bold;}")
        row1_layout.addWidget(manual_distance_btn)
            ...
            try:
                ...
                for _, row in df.iterrows():
                    try:
                        y = float(row[0])  # 1열 (Y)
                        x = float(row[1])  # 2열 (X)
                        all_points.append((x, y))  # 내부적으로는 (x, y) 순서
                    except (ValueError, IndexError):
                        continue
                
                # 중복 좌표 제거 (set을 사용하여 중복 제거)
                self.excel_points = list(set(all_points))
                original_count = len(all_points)
                unique_count = len(self.excel_points)
                
                if original_count != unique_count:
                    duplicates_removed = original_count - unique_count
                    logger.info(f"📍 엑셀 중복 제거: 원본 {original_count}개 → 유효 {unique_count}개 (중복 제거: {duplicates_removed}개)")
                else:
                    logger.info(f"📍 엑셀 중복 없음: {unique_count}개 좌표 로드")
                ...
                if original_count != unique_count:
                    ...
                    QMessageBox.information(
                        self, "완료", 
                        f"엑셀에서 {unique_count}개 좌표를 불러왔습니다.\n"
